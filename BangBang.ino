...
int desiredTemp = 40;          // Desired temperature in Fahrenheit
...
void loop() {
    ...
    if (clkState != lastClkState) { // Only process if the CLK state has changed
        ...
        if (adjustingMotorRPM) {
            ...
            desiredTemp = constrain(desiredTemp, 40, 120);
...
void displayDataPage() {
    ...
    float temperatureF = voltage / 0.01;

    // Read pH value
    int phValue = analogRead(phPin);
    float phVoltage = phValue * (5.0 / 1023.0);
    float phLevel = map(phVoltage, 0, 5, 0, 14); // Placeholder conversion

    // Read motor RPM
    int rpmValue = analogRead(motorRpmPin);
    motorRPM = map(rpmValue, 0, 1023, 0, 200);
...
void manageHeater() {
    ...
    if (abs(currentTemp - desiredTemp) > 1) {
        digitalWrite(relayPin, HIGH); // Turn on heating pad
        heaterOn = true;
    } else {
        digitalWrite(relayPin, LOW); // Turn off heating pad
        heaterOn = false;
    }
}
